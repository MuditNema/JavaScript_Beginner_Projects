...
function ShowCity() {
    ...
    xhr.open("GET", `http://api.openweathermap.org/data/2.5/weather?q=${city},in&appid=c9eac82f671439a9269d8824959db78f`, true);
    xhr.send()
    let forecast = new XMLHttpRequest();
    forecast.open("GET", `http://api.openweathermap.org/data/2.5/forecast?q=${city},in&appid=c9eac82f671439a9269d8824959db78f`, true);
    forecast.send()
    xhr.onload = function () {
        if(xhr.status === 200){
            ...
            document.getElementById('description-image').src = "http://openweathermap.org/img/w/" + res.weather[0].icon + ".png"
            ...
            forecast.onload = function () {
                ...
                let arr = [9,17,25,33];
                for(i=0;i<4;i++){
                    cl[i].innerHTML = ` <span> ${(res.list[arr[i]].main.temp - 273.15).toFixed(2)} &deg;C </span> <img style="height:4vw;width:4vw" src="http://openweathermap.org/img/w/${res.list[arr[i]].weather[0].icon}.png">`;
                }
                for(i=0;i<4;i++){
